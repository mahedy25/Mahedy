...
export default function Description() {
  ...
  useGSAP(
    () => {
      const items = gsap.utils.toArray<HTMLElement>('.reveal-text')
    ...
    >
      <div className='max-w-[1400px] w-full grid grid-cols-1 md:grid-cols-3 gap-8 items-center'>
        ...
        <div className='col-span-2 flex flex-col gap-6'>
          <p className='reveal-text text-[20px] sm:text-[24px] md:text-[32px] font-medium leading-[1.4]'>
            {fullPhrase}
          </p>
          <p className='reveal-text text-[14px] sm:text-[16px] md:text-[18px] font-light'>
            {fullDescription}
          </p>
        </div>
        ...
        <div className='reveal-text hidden md:flex justify-center md:justify-end'>
          <Link href='/about'>
            <button className='text-lg md:text-xl cursor-pointer font-semibold text-white bg-[#004D4D] rounded-full w-[120px] h-[120px] md:w-[150px] md:h-[150px] flex items-center justify-center transition-transform duration-300 hover:scale-105'>
              About Me
            </button>
          </Link>
        </div>
